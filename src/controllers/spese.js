        ...
        conn.query(query, (err, rows, fields) => {
            ...
            if (rows.length >= 1) {
                ...
                for (let i = 0; i < rows.length; i++) {
                    result.push({ id_spesa: rows[i].id_spesa, data_ora: rows[i].data_ora, spesa: rows[i].spesa, descrizione_spesa: rows[i].descrizione, id_tipo: rows[i].id_tipo, descrizione_tipo: rows[i].descrizione_tipo, tipo_movimento: rows[i].tipo_movimento })
                }
...
const newSpesa = (req, res) => {
    ...
    var body = req.body
    var query = `INSERT INTO kakebo_spese (data_ora, spesa, id_tipo, descrizione, tipo_movimento) VALUES ("${body.data}", "${body.spesa}", "${body.id_tipo}", "${body.descrizione}", "${body.tipo_movimento}")`
...
const deleteSpesa = (res, req) => {
    console.log('deleteSpesa')
    res.status(400).json({ ok: 'false', debug: 'TODO' })
}
